'''
Play against the AI and it can learn while playing.
No exploration though.
'''
SIZE = 1000
...
def win(winner,puck):
  global ai_wins,player_wins,striker2,striker1
  puck.rect.center = (SIZE/2,SIZE/2)
  puck.speed = [0,0]
  striker1.rect.center = (3*SIZE/4+100,SIZE/2)
  striker1.speed = [0,0]
  striker2.rect.center = (SIZE/4-100,SIZE/2)
  striker2.speed = [0,0]
  print(winner, "won")
  if winner == "ai":
...
class Striker(pygame.sprite.Sprite):
  ...
  def ai_move(self,puck):
    ...
    self.obs = puck_center + self_center

    self.action = np.argmax(q_table[self.obs])
...
while RUNNING:
  ...
  puck_center = striker2.get_discrete_state(puck.rect.center, SIZE)
  self_center = (striker2.get_discrete_state([striker2.rect.centerx], SIZE/2) + striker2.get_discrete_state([striker2.rect.centery], SIZE))
  new_obs = puck_center + self_center
  assert all(idx < 30 for idx in new_obs), f"{new_obs} contains an index > 29 \npuck : {puck.rect.center}\nstriker : {striker2.rect.center}"
  max_future_q = np.max(q_table[new_obs])
  current_q = q_table[striker2.obs][striker2.action]
  ...
  q_table[striker2.obs][striker2.action] = new_q
...
if create_pickle == True:
  number = int(q_table_file[5]) + 1
  with open(f"Docs/{number}AH_table.pickle", "wb") as f:
      pickle.dump(q_table, f)
  print("Crated Docs/{number}AH_table.pickle")